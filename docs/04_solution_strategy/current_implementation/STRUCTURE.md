...
├── relay_main.py       # Voice-relay entry point — SECOND Cloud Run service (Voice Companion)
├── Makefile
├── readme.md
├── requirements.txt
...
│   ├── lelik_persona_service.py # 🆕 Lelik's warm call-start prompt (full facts minus denylist, directives, primary-channel history)
...
-   **`openai_realtime_adapter.py`**: 🆕 `OpenAIRealtimeAdapter(RealtimeSessionPort)` — one live speech-to-speech call over OpenAI's Realtime WebSocket (`gpt-realtime-2.1`), μ-law 8 kHz both directions. Normalizes provider events into `RealtimeSessionEvent` (`audio_delta`, `tool_call`, `speech_started`/`speech_stopped`, `response_created`/`response_done`, `user_transcript`/`model_transcript`, `error`) so the service layer never sees provider JSON. `cancel_response()` is barge-in's provider half. Module-level `_flatten_usage` flattens OpenAI's *nested* `response.done` usage into the flat billing legs `calculate_realtime_cost` prices — non-obvious: `cached_tokens` and `reasoning_tokens` are both **subsets**, so cached is subtracted from the input legs and reasoning is never added to the output leg; a payload with no `*_token_details` at all passes through unchanged and logs a warning (it prices at $0.00).
-   **`twilio_telephony_adapter.py`**: 🆕 `TwilioTelephonyAdapter(TelephonyPort)` — outbound call origination via the Twilio REST API, with answering-machine detection mandatory. `Client.calls.create` is synchronous (`requests`), so it is offloaded through `asyncio.to_thread` rather than blocking the 1-vCPU main service's event loop.
-   **`http_call_control_plane_adapter.py`**: 🆕 `HttpCallControlPlaneAdapter(CallControlPlanePort)` — **relay-side only**; the relay's sole edge back to the main service (`/voice/session-config`, `/voice/submit-transcript`), authenticated with a Google OIDC token minted from the relay's own service identity. Lives in `adapters/` rather than `services/` specifically so REQ-ARCH-18's `httpx` ban does not apply.
...
-   **`user_agent_factory.py`**: 🆕 `UserAgentFactory(AgentFactoryPort)` — per-user agent instance factory. Moved from `services/` to `composition/` (legal to import all layers). Caches agent sets per user (1h TTL). Resolves 3-level config (USER > ACCOUNT > SYSTEM) during instantiation. Two-tier lifecycle: **eager agents** (Router, Quick, Smart, Memory, WebSearch, etc.) created on first request; **lazy agents** (DocGenerator, PdfGenerator, DeepResearch, FileManagement, etc.) created on first delegation via `AgentFactoryPort.create_agent_on_demand()`. Lazy agents tracked in `_lazy_agent_ids` for eviction.
...
-   **`voice_playback_tracker.py`**: 🆕 `PlaybackTracker` — what the caller has actually heard. Outbound chunks are counted in μ-law bytes and each is followed by a Twilio `mark` named by the running total; echoed marks advance `played_bytes` (8 bytes = 1 ms). Read by barge-in truncation and the silence watchdog.
-   **`voice_call_buffer.py`**: 🆕 `VoiceCallBuffer` + `VoiceTurnSegment` — call-scoped accumulator held by `VoiceSessionService` and flushed once at call end. Turns are segmented on the provider's `response.created` → `response.done` boundary; `usage_by_model` keys usage per model that actually ran, matching the repo's per-model pricing rule.
-   **`voice_audio_frame.py`**: 🆕 `AudioFrame` — a carrier-agnostic slice of call audio crossing `RealtimeSessionPort`. `payload` holds Twilio's base64 μ-law string **as-is**; nothing in the pipeline decodes or re-encodes it.
-   **`lelik_context.py`**: 🆕 `LelikContext` — frozen dataclass, what Lelik knows at call start (`biographical_facts`, `conversation_history`): the output of `LelikPersonaService.assemble()`, consumed by `LelikAgent.session_config()` to build the realtime session's prompt. Pure data, zero external deps — the context/prompt-authoring split the Slice 2 rewrite of `LelikPersonaService` introduced (VOICE_COMPANION_RFC §4.7/§4.8).
...
-   **`media_stream_handler.py`**: 🆕 `MediaStreamHandler` — one Twilio Media Streams WebSocket connection, **relay-side**. Extracts the call ticket from the `start` event's `customParameters` and drives `VoiceSessionService.handle_call` for the stream's lifetime. Per REQ-ARCH-25 it imports no ports — only `AudioFrame` (domain) and the service. Carries Twilio's already-base64 μ-law payloads through untouched (no decode/re-encode). Supplies two closures over `stream_sid`/`ws`: `send_outbound` (a media frame followed by a Twilio `mark` named by the running byte total) and `clear_outbound_audio` (the `{"event": "clear"}` half of barge-in). It owns the call's `PlaybackTracker`: sent chunks and echoed `mark`s feed it, and the session loop reads it. Cleanup lives in a single `finally`, not the `stop` branch — `websockets` swallows `ConnectionClosedOK` inside `__aiter__`, so a clean close ends the loop with no exception and no `stop` event at all.
-   **`learning_loop.py`**: ⚠️ LEGACY.
...
**Voice Companion Ports:**
-   **`realtime_session_port.py`**: 🆕 `RealtimeSessionPort` ABC + `RealtimeSessionEvent` value object — one live speech-to-speech session (`open`, `send_audio`, `receive_events`, `submit_tool_result`, `submit_message`, `request_response`, `cancel_response`, `close`). Adapter: `OpenAIRealtimeAdapter`. Two caller responsibilities the ABC states explicitly because the provider errors otherwise: do not `request_response()` while one is active, and do not `cancel_response()` when none is. `PROMPT_CACHE_BOUNDARY` must be stripped from `instructions` before `open()`.
-   **`telephony_port.py`**: 🆕 `TelephonyPort` ABC — `originate_call(to, from_, answer_url, status_callback_url)` → provider call SID. Adapter: `TwilioTelephonyAdapter`. Twilio dialing *in* is a handler; us telling Twilio to place a call is this outbound port.
-   **`call_control_plane_port.py`**: 🆕 `CallControlPlanePort` ABC — the relay's only edge to the main service: `fetch_session_config(ticket)` and `submit_transcript(...)`. Adapter: `HttpCallControlPlaneAdapter`. No retry on submit failure by design (fail loudly, never re-run a call's billing).
...
-   **`lelik_persona_service.py`**: 🆕 `LelikPersonaService` — builds Lelik's call-start prompt (Voice Companion RFC §4.8, `decisions/lelik_warm_context.md`): the whole biographical cache minus `UserBotConfig.voice_excluded_fact_domains`, standing directives, the last 30 primary-channel messages as stored summaries (read via `UserNotificationService.resolve_channel`, the chain the call summary is written through), date/time and location. Constructed per call in `main.py` with a `UserPromptBuilder` over the caller's own config. Raises on failure, so the webhook fails closed.
-   **`voice_session_service.py`**: 🆕 `VoiceSessionService` — the relay's per-call session loop (Voice Companion RFC §4.5). Redeems the ticket via `CallControlPlanePort`, opens a `RealtimeSessionPort`, then runs inbound forwarding and outbound event consumption as two genuinely concurrent tasks (`asyncio.wait(FIRST_COMPLETED)`). Accumulates turns + per-model usage into a `VoiceCallBuffer` flushed exactly once at call end. Deliberately **not** an agent (REQ-ARCH-03/-30 forbid the `*Agent` name here) and provider-agnostic — it labels usage with the model the *port* reports, never a hardcoded string. `session.open()` sits inside the `try/finally` so even a failed connect still reaches `close()` + `submit_transcript()`, releasing the one-call marker immediately instead of by TTL. Owns **barge-in policy** (the provider's auto-interrupt is off): on `speech_started` while a response is active, it reads the heard milliseconds of the current assistant item from `PlaybackTracker` *before* clearing, then runs clear → cancel → `truncate(item_id, heard_ms)`. Guarded, because cancelling with nothing in flight is a provider error. It also runs a **silence watchdog**: after playback has finished, with no response active and the caller quiet, `silence_timeout_s` (default 8) of silence injects one system note plus `response.create`, re-armed only when the caller speaks.
...
-   **`lelik_agent.py`**: 🆕 `LelikAgent` — the voice companion, and as of Slice 2 a standard delegating agent with three entry points: `execute(purpose, ticket, answer_url, to_number)` places the callback (`TelephonyPort.originate_call`, no LLM call); `session_config(user_id, account_id)` builds the realtime session's instructions (over `LelikContext`, RFC §4.8) and tool declaration; `delegate(user_id, account_id, arguments, call_context)` runs one `delegate_to_specialist` call from the live session through `DelegationEngine.dispatch` (public for this second caller). `internal=True`, `capabilities={}`, no `Intent` of its own — still unreachable via `delegate_to_specialist` or `AgentCoordinator`, `can_handle()` still `False` unconditionally — but `LELIK.allowed_intents={search_memory, search_web, ask_alek}` (`docs/04_solution_strategy/decisions/allowlist_names_internal_intent.md`). Built by `UserAgentFactory._build_lelik`, reached via `UserAgentFactory.get_lelik(user_id)` (refreshes the per-user cache TTL, keeping the Router warm for `ask_alek`). Call sites: `voice_webhook_app.py`'s `/voice/inbound-status` (`execute()`) and `/voice/answer` (`session_config()`), `voice_control_plane_app.py`'s `/voice/delegate` (`delegate()`). Non-obvious: it deliberately does **not** catch origination failures — the webhook's own `except` is what deletes the ticket and releases the one-call marker.
-   **`lelik_summarizer_agent.py`**: 🆕 `LelikSummarizerAgent` — Lelik's extractor, the slot `TutorExtractorAgent` fills for Tutor. Not manifest-registered; constructed fresh per call by `composition/companion_extractor_runner.py` (`companion_type="voice"`) and reached from `main.py`'s `_voice_summary_consumer` on `/voice/submit-transcript`, ending at `UserNotificationService.notify_call_summary`. Single LLM call, **plain text out, not JSON** — Lelik has no `CompanionRecord` store, so `records` is hardcoded `[]` and never asked of the model. Tier `ECO`, explicitly pinned in `_DEFAULT_AGENT_TIERS` precisely *because* the agent is invisible to `test_every_llm_agent_has_a_default_tier`; with no `AgentProviderStrategy` entry, provider falls back to `STRATEGIES["quick"]`.
-   **`alek_gateway_agent.py`**: 🆕 `AlekGatewayAgent` — Alek as a specialist behind `Intent.ASK_ALEK`, zero-LLM (VOICE_COMPANION_RFC §4.7). `internal=True`, `eager=False`, named only by `LELIK.allowed_intents`. `execute()` routes to `router_agent_{user_id}` (not Smart directly — `enrich_context`/RRF memory search runs in the Router), passing the commission text via `context["current_message_parts"]`. `RETRY_POLICY = NO_RETRY_POLICY` — a retry would re-run Router+Smart+specialists (double spend, double chat copy). Cancels any `response_summary_task` Smart scheduled (nothing on this path writes history, so the summary would be paid for and dropped) and copies reading-shaped answers (`link_list`/`structured_data`) to chat via `UserNotificationService.notify_answer_copy` (RFC §4.10 rule 2).
...
-   **`voice_webhook_app.py`**: 🆕 Twilio's four entry points for a "call Lelik" session, all HMAC-verified via the injected `signature_verifier`. `POST /voice/auth` — resolve `From` → user (platform `"phone"`), enforce the one-call-per-user marker, mint a short-TTL ticket, and park the callback under the inbound `CallSid` (an unbound number gets no ticket, no marker, no call). `POST /voice/inbound-status` — on the inbound dial's `completed`, atomically claim the parked callback and have `LelikAgent` originate it. Originating any earlier hit the dial's teardown and the carrier returned SIP 480. `POST /voice/answer` — resolve the caller's `LelikAgent` via the injected `lelik_agent_provider` (`UserAgentFactory.get_lelik`) and await `session_config()` for the warm call-start prompt (RFC §4.8), then return TwiML pointing Twilio's Media Stream at the relay. `POST /voice/status` — the callback leg's lifecycle; it is the *only* thing that can release the ticket and marker when a call rings out, is busy, or fails at the carrier, because `/voice/answer` is never reached in those cases. The ticket travels in the **query string**, which is why the signature scheme covering the full URL matters.
-   **`voice_control_plane_app.py`**: 🆕 The relay-facing half, OIDC-protected like `/worker` (verifier injected, so the local-dev bypass policy stays in `main.py`). `POST /voice/session-config` — exchange the ticket for session config; **atomically consumed** via `get_and_delete`, so a captured ticket cannot be replayed within its TTL to read back the owner's persona and facts. `POST /voice/submit-transcript` — record per-model usage/cost (`calculate_realtime_cost` over the already-flattened legs), hand the transcript to the injected `summary_consumer`, and release the one-call marker — the release happens even if the other two raise, since a stuck marker locks the user out permanently.
-   **`twilio_signature_verifier.py`**: 🆕 In-app `X-Twilio-Signature` verification (HMAC-SHA1 over the full URL *including query string* plus sorted POST params). Needed because the service must stay `--allow-unauthenticated` for Slack/Telegram/OAuth/MCP and Twilio cannot present a Google OIDC token; before it existed, `/voice/auth` and `/voice/answer` were the codebase's only unauthenticated external entry points. Pure crypto check — token source and the `TWILIO_AUTH_TOKEN`-unset local bypass live in `main.py`'s wiring.
