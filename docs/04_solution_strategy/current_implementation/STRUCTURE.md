...
-   **`lelik_agent.py`**: 🆕 `LelikAgent` — the voice companion's front desk. `internal=True` with `capabilities={}` and no `Intent`, so it is unreachable via `delegate_to_specialist` *or* `AgentCoordinator`; `can_handle()` returns `False` unconditionally. **Makes no LLM call and has no tier** — `execute(purpose, ticket, answer_url)` only calls `TelephonyPort.originate_call` (Lelik's persona runs as an OpenAI Realtime session inside the relay, outside `AgentExecutionContext` entirely). Built by `UserAgentFactory._build_lelik`; the only call site is `voice_webhook_app.py`'s `/voice/auth`, which awaits `execute()` directly. Non-obvious: it deliberately does **not** catch origination failures — the webhook's own `except` is what deletes the ticket and releases the one-call marker.
