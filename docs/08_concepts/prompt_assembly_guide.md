...
5. Build `knowledge_base {}` block if either biographical_context or conversation_history is non-empty:
   - `kb_preamble=True` → placed **before** the blueprint template (conversational agents)
   - `kb_preamble=False` (default) → placed **after** the blueprint template (consolidation)
...
**Pattern A: Conversational (Smart, Quick, Router) — `kb_preamble=True`**
...
class Alek extends Agent {
  personality { ... }
  cognitive_process { ... }            ← instructions last → better recency
  protocols { ... }
  output_format { ... }
}
...
**Pattern B: Document Analysis (Consolidation) — `kb_preamble=False` (default)**
...
knowledge_base {                       ← AFTER blueprint (postamble)
  biographical_context: '''...'''

  conversation_history: '''            ← history batch to consolidate
    user: ...
    assistant: ...
  '''
}
...
**Key rules:**
- The `knowledge_base` block only appears when at least one of `biographical_context` or `conversation_history` is non-empty. No empty wrappers.
...
- `kb_preamble=True` → `knowledge_base` before blueprint (context first, instructions last → better instruction-following via recency).
- `kb_preamble=False` (default) → `knowledge_base` after blueprint (consolidation: history batch is the primary task payload, kept close to the end).

---

## 4. Agent Patterns

### Pattern A: Conversational (Smart, Quick, Router)
...
- `kb_preamble=True` → `knowledge_base {}` placed before the blueprint so cognitive_process and protocols are last (recency effect improves instruction-following)
...
|-------|----------|----------|
| `True` | `knowledge_base {}` placed **before** the blueprint template | Smart, Quick, Router — biographical context before instructions |
| `False` (default) | `knowledge_base {}` placed **after** the blueprint template | Consolidation — history batch is the task payload, stays near end |

Agents own this decision — they pass the flag explicitly. The assembly service has no knowledge of agent types.
...
`PROMPT_CACHE_BOUNDARY = "<!-- CACHE_BOUNDARY -->"` is defined in `src/ports/llm_port.py`.
...
| `src/ports/llm_port.py` | `PROMPT_CACHE_BOUNDARY` constant |
