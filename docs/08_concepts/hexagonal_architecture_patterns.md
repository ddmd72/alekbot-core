...
- **UserAgentFactory** (`services/`) — per-user. Creates agents per user, using shared services injected from ServiceContainer. Does NOT import concrete adapters.
...
```python
# UserAgentFactory — src/services/user_agent_factory.py
# NO adapter imports — receives everything via DI from ServiceContainer
...
# src/ports/llm_service.py — re-export for backward compat
...
# from ..ports.llm_service import Message  # ❌ port→port violation!
```
...
```python
# ❌ WRONG — session_store.py (port) imports from llm_service.py (another port)
# src/ports/session_store.py
from ..ports.llm_service import Message  # port→port dependency!
...
### Violation 5: isinstance() Type Check in Services Layer
...
Composition/wiring → composition/ (ServiceContainer) or services/ (UserAgentFactory)
...
| Composition Root Split | ServiceContainer (shared) + UserAgentFactory (per-user) | `src/composition/service_container.py`, `src/services/user_agent_factory.py` |
