...
class RouterAgent(BaseAgent):
    async def classify(self, query: str) -> RoutingDecision:
        ...
        triage_result = await self.llm_port.generate(
...
class ProviderRegistry:
    def get_provider(self, tier: PerformanceTier) -> LLMPort:
...
## 12. No Direct Environment Access in Agents

**Problem:** Agents call `os.getenv()` inline for feature flags, API keys, or configuration. This violates hexagonal boundaries (agents must not access infrastructure) and makes testing harder.

**Solution:** Two patterns depending on the type of value:

### Feature Flags → Centralize in agent_config.py

```python
# ❌ WRONG — agent reads env var directly
import os
...
# ✅ CORRECT — read once at import time from centralized config
...
### Secrets / API Keys → Constructor Injection from Composition Layer

```python
# ❌ WRONG — agent reads API key from env
class MapsSearchAgent(BaseAgent):
    def _build_widget(self, token):
        api_key = os.getenv("GOOGLE_SEARCH_API_KEY", "")
        ...

# ✅ CORRECT — injected via constructor
class MapsSearchAgent(BaseAgent):
    def __init__(self, ..., maps_js_api_key: str = ""):
        self._maps_js_api_key = maps_js_api_key

# Wired in composition/user_agent_factory.py:
MapsSearchAgent(..., maps_js_api_key=self.config.get("GOOGLE_SEARCH_API_KEY", ""))
```

**Applied to (2026-03-05):**
- `QuickResponseAgent`, `SmartResponseAgent` — `ENABLE_HISTORY_OPTIMIZATION`
- `WebSearchAgent`, `WebSearchLightAgent` — `ENABLE_GROUNDING_ATTRIBUTION`
- `MapsSearchAgent` — `maps_js_api_key` via constructor injection

**Applicability:** Mandatory for all agents

**Implemented in:**
- `src/infrastructure/agent_config.py` — centralized feature flags
- `src/composition/user_agent_factory.py` — DI wiring for secrets
...
| No Direct Env Access | 🔴 P0 | ✅ Implemented | High | Low |
...
- [x] 🚫 No direct `os.getenv()` in agents — flags in `agent_config.py`, secrets via DI
- [ ] 💾 Redis cache for repeated queries
- [ ] 📝 Audit log for all changes
- [ ] 🧪 Canary deployment strategy
- [x] 🎨 Transparent UI with status updates

**v6.0 Compliance:** 8/12 practices implemented ✅
