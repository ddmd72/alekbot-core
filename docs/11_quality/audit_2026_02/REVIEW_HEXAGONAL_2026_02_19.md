...
**Problem:** `Message`, `MessagePart`, `ToolCall` are defined in `src/ports/llm_port.py:7-23`, even though these are fundamental domain models of the conversation.

**Why this is a problem:**
- Ports (`session_store.py`, `file_service.py`) import from another port — port→port dependency:
  - `src/ports/session_store.py:7` → `from ..ports.llm_port import Message`
  - `src/ports/file_service.py:9` → `from ..ports.llm_port import MessagePart`
...
In `src/ports/llm_port.py`, keep a re-export for backward compatibility:
```python
...
| **ports/** | 80% | port→port: `session_store`→`llm_port`, `file_service`→`llm_port` | Fix 1 resolves this |
...
- `session_store.py` ──→ `llm_port.py` (port→port)
- `file_service.py` ──→ `llm_port.py` (port→port)
...
|-------|--------|-------|-----------|
| **ports/** | 80% | 80% | port→port (`session_store`/`file_service` → `llm_port`) — fix 1.1 |
