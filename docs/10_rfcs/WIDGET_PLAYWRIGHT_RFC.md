...
parse_llm_response()          → extracts RichContent(content_type="widget", ...)

ConversationHandler           → routes to RichContentService.process()

RichContentService            → _handle_widget()
...
which is sent as a message; Slack/Telegram unfurls the URL). `widget` uses
...
- **Lazy init** — browser starts on first `render()` call, not at container startup.
  Cold start does not pay Chromium launch cost (~1–2s); only the first `widget`
  request pays it.
...
`"widget"` branch in `process()` → `_handle_widget()`.
...
`process()` returns `None` for `widget` — no URL, upload is direct.

---

## 8. ConversationHandler

`"widget"` added to `_MEDIA_CONTENT_TYPES`:
...
(inside `_handle_widget`) happens. Text was already delivered before this point.
...
When `false`: `HtmlRendererPort` is `None` → `widget` content is silently skipped.
...
- Cold start: +~1–2s on first `widget` request (lazy init)
- With `min-instances=0`: no cold start on container startup; only first widget request
...
4. FORMAT: Before writing anything — decide the delivery format.
   Defaults: weather → widget. Prices/rates → widget.
   Multi-day or multi-item data → widget.
...
5. DRAFT: Write full_response AND HTML simultaneously (if FORMAT decided widget).
```
...
to text format. Moving the decision before drafting makes widget a primary output
...
- `tests/unit/services/test_rich_content_widget.py` — 5 service tests:
...
| `src/services/rich_content_service.py` | **Modified** — `_html_renderer`, `_handle_widget()` | ✅ |
| `src/handlers/conversation_handler.py` | **Modified** — `"widget"` in `_MEDIA_CONTENT_TYPES` | ✅ |
...
| `tests/unit/services/test_rich_content_widget.py` | **Created** — service tests | ✅ |
| `scripts/test_html_render.py` | **Created** — local smoke test | ✅ |
| `firestore_utils/uploads/OUTPUT_FORMAT_JSON.groovy` | **Modified** — `widget` type + iOS widget style + grid layout | ✅ |
...
| Q4 | Multiple `widget` per response (future)? | Not in V1 — single `rich_content`. Extend later if needed. |
