...
Location-based queries ("знайди аптеку поблизу", "де поїсти японську кухню в центрі") are
...
```
User: "знайди кафе поблизу мого офісу що відкрите зараз"
  │
  ↓ Smart/Quick → maps_query intent → MapsSearchAgent
      │    payload: {"query": "знайди кафе поблизу мого офісу що відкрите зараз"}
      │
  ...
  ↓ Smart LLM receives tool result with token
      → generates HTML page on its own design judgment:
          <html>
            ...
            <body>
              <h2>Кафе поблизу офісу</h2>
              <gmp-place-contextual context-token="eyJ...">
              </gmp-place-contextual>
              <p>Текстовий коментар від Smart</p>
            </body>
          </html>
      → deliver_response(
            full_response="текстова відповідь для history",
            rich_content={
                "type": "html_file",
                "data": {"html": "<html>...", "filename": "map_{uuid}.html"},
                "fallback": "текстова відповідь з адресами"
            }
        )

  ↓ ConversationHandler
      → upload HTML file до GCS (Content-Type: text/html, Cache-Control: no-store)
      → GCS публічний URL: https://storage.googleapis.com/{bucket}/maps/{uuid}.html
      → Slack/Telegram: відправляє URL як посилання ("📍 Відкрити карту")
      → юзер клікає → браузер відкриває живу інтерактивну карту
      → fallback: якщо GCS upload недоступний → text з адресами
```
