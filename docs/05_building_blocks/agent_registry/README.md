...
- `src/adapters/claude_adapter.py` — Claude adapter. Model tiers: ECO=`claude-haiku-4-5-20251001`, BALANCED=`claude-haiku-4-5-20251001`, PERFORMANCE=`claude-sonnet-4-6`, ULTRA=`claude-opus-4-7`. Adaptive thinking + `output_config.effort` are gated on `_THINKING_MODELS = ("claude-sonnet", "claude-opus")` — both parameters are silently dropped when the resolved model is Haiku (verified against `models.retrieve(...).capabilities.effort.supported`; Haiku 4.5 returns `False` and the API rejects with HTTP 400). `temperature` auto-set to 1.0 when thinking is active. `max_tokens` reads from `LLMRequest.max_tokens` (default 16,000). SDK pin: `anthropic >= 0.97.0`.
