...
Placeholder for future LLM-based semantic risk assessment. Currently delegates all
validation to an injected `SecurityPort` fallback (typically `RegexSecurityAdapter`).
Accepts fallback via constructor DI — no direct adapter-to-adapter imports.

### 4.4 ExternalAPIAdapter (Placeholder)

Placeholder for external content safety API integration (e.g., Google Perspective,
Azure Content Safety). Currently delegates to injected `SecurityPort` fallback.
Accepts `fallback`, `api_url`, and `api_key` via constructor — no direct adapter imports.
...
- `src/ports/security_port.py`: `SecurityPort` ABC interface.
...
- `src/adapters/security/llm_adapter.py`: LLM-based validation (placeholder, DI fallback).
- `src/adapters/security/external_api_adapter.py`: External API validation (placeholder, DI fallback).
