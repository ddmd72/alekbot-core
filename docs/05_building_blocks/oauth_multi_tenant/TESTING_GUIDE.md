...
**Use Case:** You already have a user (e.g. U_DMYTRO_CORE) and want to link Google OAuth to it.
...
### Option B: Manual migration (for U_DMYTRO_CORE)
...
3. **Copy external_user_id to U_DMYTRO_CORE:**
   ```python
   # Get your existing user
   dmytro = db.collection("dev_users_oauth").document("U_DMYTRO_CORE").get()
   ...
   # Save
   db.collection("dev_users_oauth").document("U_DMYTRO_CORE").set(dmytro_data)

   print("✅ OAuth linked to U_DMYTRO_CORE")
   ```
...
5. **Verify you can now log in via OAuth:**
   ```bash
   open http://localhost:5000/auth/login
   # Should log in as U_DMYTRO_CORE
   ```
