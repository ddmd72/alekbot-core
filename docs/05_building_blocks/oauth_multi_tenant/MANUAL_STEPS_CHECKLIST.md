...
### Step 7: Link OAuth to YOUR_USER_ID (5 minutes)
...
3. Copy into YOUR_USER_ID:
   ```python
   # Get existing user
   dmytro_doc = db.collection("dev_users_oauth").document("YOUR_USER_ID").get()

   if not dmytro_doc.exists:
       print("❌ YOUR_USER_ID not found in dev_users_oauth")
       ...
       # Save
       db.collection("dev_users_oauth").document("YOUR_USER_ID").set(dmytro)

       print("✅ Google OAuth linked to YOUR_USER_ID")
       print(f"✅ external_user_id: {external_user_id}")
   ```
...
5. Verify that you can now log in as YOUR_USER_ID:
   ```bash
   open http://localhost:5000/auth/login
   # Should log in as YOUR_USER_ID with your email
   ```

**Result:** ✅ YOUR_USER_ID is now linked to Google OAuth!
...
# Get your user
user = await user_repo.get_user("YOUR_USER_ID")
