...
5. Append `current_date_time {}` (always)
6. Append `query_specific_context` (only if Q-S facts present)
...
`src/services/user_prompt_builder.py` — `UserPromptBuilder` implements `PromptBuilderPort`.
