...
- **PDF Generator Agent:** PDF creation specialist. Called by both Quick and Smart via `create_pdf` intent (`ExecutionMode.ASYNC` → Cloud Task dispatch). BALANCED tier (Gemini, `agent_type="pdf_generator"`, `internal=False`). Single LLM call: natural language → complete HTML+CSS document. System prompt embeds a style catalogue (12 design systems); LLM auto-selects the appropriate style. HTML rendered to PDF via `PuppeteerRunnerPort` (Node.js subprocess, `pdf_generator/runner.js`). Filename extracted from `<title>` tag. Returns two `DeliveryItem("document", ...)` items — HTML (`file_upload=False`, GCS only) and PDF (`file_upload=True`, GCS + Slack upload). See [Document Generation Building Block](../document_generation/README.md).
