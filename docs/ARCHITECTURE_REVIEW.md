...
- `src/ports/llm_service.py` — defines DTOs (`Message`, `MessagePart`, `LLMRequest`, `LLMResponse`) that are essentially domain value objects but live in ports. Because of this, `file_service.py` and `session_store.py` import from a sibling port instead of domain.
...
- **Agents** — depend on ports (`LLMService`, `FactRepository`, `EmbeddingService`). No violations.
...
| P2 | Architecture | `ports/llm_service.py` | DTOs in port instead of domain |
