...
| ------------------- | ---------------------------- | -------------------------- |
| `LLMService`        | `GeminiAdapter`              | Google Gemini API          |
| `LLMService`        | `ClaudeAdapter`              | Anthropic Claude API       |
...
5. **Reasoning:** Agents call the `LLMService` (Port) to generate responses, which are fulfilled by the `GeminiAdapter` or `ClaudeAdapter`.
