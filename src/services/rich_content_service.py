...
Supported types:
  ...
  html_card — Agent-generated HTML → Playwright screenshot → PNG → platform upload (inline image)
...
class RichContentService:
    ...
    async def process(self, content: RichContent, channel_id: str) -> Optional[str]:
        ...
        elif content.content_type == "html_card":
            await self._handle_html_card(content, channel_id)
            return None
        else:
            logger.warning(
