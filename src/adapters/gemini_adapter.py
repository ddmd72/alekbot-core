...
class GeminiAdapter(LLMPort):
    ...
    async def generate_content(
        ...
        # Sandboxed Python code execution — Gemini-native, injected transparently.
        # Other providers set use_code_execution=False (ignored at LLMRequest level).
        if use_code_execution:
            code_exec = types.Tool(code_execution=types.ToolCodeExecution())
            tools = [code_exec] + (tools or [])
