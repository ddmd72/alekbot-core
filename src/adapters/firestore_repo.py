...
class FirestoreFactRepository(FactRepository):
    ...
    async def get_longest_facts(self, account_id: str, limit: int) -> List[FactEntity]:
        """Return top-K active facts sorted by word count descending (client-side sort)."""
        facts = await self.get_active_facts(account_id)
        sorted_facts = sorted(facts, key=lambda f: len(f.text.split()), reverse=True)
    ...
    async def get_biographical_context_cached(
        ...
        if owner_id is None:
            ...
            logger.debug(f"🔍 [Cache] Auto-resolved owner_id from RequestContext: {(owner_id or '')[:12]}...")
