#!/usr/bin/env python3
"""
Force regenerate ALL vectors in production 'facts' collection using RETRIEVAL_DOCUMENT.
"""
import asyncio
import sys
import os
...
async def main():
    print("🔄 Force regenerating ALL vectors in PRODUCTION 'facts'...")
    ...
    # Get production facts collection
    facts_col = db_client.collection('facts')

    print("🔍 Fetching all documents from 'facts'...")
    docs = []
    async for doc in facts_col.stream():
        docs.append(doc)
    ...
    for doc in docs:
        try:
            ...
            # Update document
            await facts_col.document(doc.id).update({
    ...
    print(f"\n🎉 Production force regeneration complete!")
    print(f"   Updated: {fixed}/{len(docs)} documents")
