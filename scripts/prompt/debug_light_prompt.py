...
async def main():
    print("⏳ Initializing components (LIGHT mode)...")
    ...
    # 3. Build Components (LIGHT)
    print("⏳ Fetching light prompt components...")
    components = await prompt_builder.build_system_prompt(mode="light")
    ...
    light_prompt = brain._format_light_prompt(components)

    # 5. Output
    reports_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "reports", "prompt")
    os.makedirs(reports_dir, exist_ok=True)
    output_file = os.path.join(reports_dir, "debug_light_prompt_output.groovy")
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(light_prompt)

    print("\n✅ Light System Prompt generated successfully!")
    print(f"📂 Saved to: {output_file}")
    print(f"📊 Size: {len(light_prompt)} characters")
    print("-" * 80)
    print(light_prompt)
    print("-" * 80)
