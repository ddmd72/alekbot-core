...
async def inspect_quick_prompt(
    user_id: str,
    session_id: Optional[str] = None
):
    print(f"\n⚡ INITIALIZING QUICK PROMPT INSPECTION")
    ...
    report_path = f"reports/prompt/{date_part}-quick-{user_short}-{time_part}.md"
    os.makedirs("reports/prompt", exist_ok=True)
    
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(system_prompt)
        
    print(f"\n✨ DONE! Quick Agent prompt saved to: {report_path}")
    print("\n--- PREVIEW ---")
    print(system_prompt[:1000] + "..." if len(system_prompt) > 1000 else system_prompt)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inspect Quick Response Agent Prompt with Real Data")
    ...
    asyncio.run(inspect_quick_prompt(
        user_id=user_id
    ))
