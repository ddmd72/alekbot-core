...
class TestGracefulDegradationFallback:
    ...
    async def test_double_failure_shows_synthetic_apology(self):
        """Both SmartAgent and QuickAgent fail → synthetic apology shown, no ERROR status."""
        handler, coordinator, session_store = make_handler()
        channel = make_channel()
        context = make_context()
        ...
        assert coordinator.route_message.call_count == 2
        # Synthetic apology sent as normal text — user never sees ERROR status
        channel.send_chunked_message.assert_called_once()
        channel.send_status.assert_not_called()

    async def test_quickagent_exception_shows_synthetic_apology(self):
        """QuickAgent raises Exception → caught, synthetic apology shown, no ERROR status."""
        handler, coordinator, session_store = make_handler()
        channel = make_channel()
        context = make_context()
