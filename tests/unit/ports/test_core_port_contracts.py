...
Ports covered (19):
  ...
  InviteCodeRepository, LLMPort, LogSink (Protocol),
...
class TestLLMPortContract:
    ...
    def test_has_generate_content(self):
        assert getattr(LLMPort.generate_content, "__isabstractmethod__", False)

    def test_has_supports_caching(self):
        assert getattr(LLMPort.supports_caching, "__isabstractmethod__", False)

    def test_has_upload_file(self):
        assert getattr(LLMPort.upload_file, "__isabstractmethod__", False)

    def test_has_get_capabilities(self):
        assert getattr(LLMPort.get_capabilities, "__isabstractmethod__", False)

    def test_has_get_model_for_tier(self):
        assert getattr(LLMPort.get_model_for_tier, "__isabstractmethod__", False)

    def test_all_abstract_methods_count(self):
        """Port should have exactly 5 abstract methods."""
        abstract_methods = {
            name for name, method in inspect.getmembers(LLMPort)
            if getattr(method, "__isabstractmethod__", False)
        }
        assert len(abstract_methods) == 5, f"Expected 5 abstract methods, got {abstract_methods}"

    def test_generate_content_is_async(self):
        assert inspect.iscoroutinefunction(LLMPort.generate_content)

    def test_supports_caching_is_sync(self):
        assert not inspect.iscoroutinefunction(LLMPort.supports_caching)

    def test_get_capabilities_is_sync(self):
        assert not inspect.iscoroutinefunction(LLMPort.get_capabilities)

    def test_get_model_for_tier_is_sync(self):
        assert not inspect.iscoroutinefunction(LLMPort.get_model_for_tier)
...
class TestLLMPortMockImplementation:
    @pytest.fixture
    def mock_service(self):
