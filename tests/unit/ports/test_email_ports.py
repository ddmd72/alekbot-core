...
from src.domain.email import EmailMetadata, EmailClassificationResult
...
class TestEmailClassifierPortContract:
    """Verify EmailClassifierPort port declares all required abstract methods."""

    def test_is_abstract_class(self):
        assert issubclass(EmailClassifierPort, ABC)

    def test_cannot_instantiate_directly(self):
        with pytest.raises(TypeError):
            EmailClassifierPort()

    def test_has_classify_batch(self):
        assert getattr(EmailClassifierPort.classify_batch, "__isabstractmethod__", False)

    def test_all_abstract_methods_count(self):
        assert _abstract_methods(EmailClassifierPort) == {"classify_batch"}

    def test_classify_batch_is_async(self):
        import inspect
        assert inspect.iscoroutinefunction(EmailClassifierPort.classify_batch)

    def test_classify_batch_signature(self):
        params = _params(EmailClassifierPort, "classify_batch")
        assert params == ["self", "emails", "user_id", "credentials"]

    def test_classify_batch_credentials_default_none(self):
        import inspect
        sig = inspect.signature(EmailClassifierPort.classify_batch)
        assert sig.parameters["credentials"].default is None


class TestEmailClassifierPortMockImplementation:
    """Verify AsyncMock(spec=EmailClassifierPort) satisfies the port contract."""

    @pytest.fixture
    def mock_classifier(self):
        return AsyncMock(spec=EmailClassifierPort)

    async def test_classify_batch_returns_list(self, mock_classifier):
        mock_classifier.classify_batch.return_value = []
        result = await mock_classifier.classify_batch(emails=[], user_id="u1")
        assert isinstance(result, list)

    async def test_classify_batch_called_with_credentials(self, mock_classifier):
        mock_classifier.classify_batch.return_value = []
        await mock_classifier.classify_batch(emails=[], user_id="u1", credentials=None)
        mock_classifier.classify_batch.assert_called_once_with(
            emails=[], user_id="u1", credentials=None
        )
