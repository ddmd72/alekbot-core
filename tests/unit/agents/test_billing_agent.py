...
class TestBillingAgentConcurrency:
    ...
    async def test_concurrent_flush_does_not_double_pop(self):
        ...
        # record_usage should be called exactly once with all 10 tokens
        # (second flush gets empty list → returns early)
        total_calls = quota_service.record_usage.await_count
        if total_calls == 1:
            call_kwargs = quota_service.record_usage.await_args.kwargs
            assert call_kwargs["tokens"] == 10
        else:
            # If called twice, combined tokens must still equal 10 (no double-count)
            calls = quota_service.record_usage.await_args_list
            total = sum(c.kwargs["tokens"] for c in calls)
            assert total == 10
