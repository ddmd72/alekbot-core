...
async def test_consolidate_command_trigger():
    ...
    # We need to patch the function where it is DEFINED, because it is imported locally
    with patch("src.handlers.consolidation_handler.process_user_batches_on_overflow", new_callable=AsyncMock) as mock_consolidation:
        handler.consolidation_queue = AsyncMock()
        
        # Correctly mock the async method chain
        ...
        # In the new flow, handle_command enqueues the batch and triggers the background processing
        # So we assert that the background task was called
