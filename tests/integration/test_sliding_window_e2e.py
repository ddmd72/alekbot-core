...
from src.domain.user import UserProfile, UserBotConfig, UserTier, LLMProvider
...
async def test_sliding_window_e2e():
    ...
    try:
        ...
        if not user:
            ...
            user = UserProfile(
                ...
                config=UserBotConfig(
                    ...
                    smart_model="models/gemini-3-pro-preview" # Ensure high-quality consolidation
                ),
                tier=UserTier.ADMIN
            )
        ...
        await cleanup_test_data(TEST_USER_ID, db_client, env_config.firestore_collection_prefix, report, True)
        
        # Initialize Core Components
        coordinator = AgentCoordinator()
        llm_service = GeminiAdapter(api_key=config["GEMINI_API_KEY"])
        file_service = FileUploadService(llm_service)
        ...
        # Verify Facts
        active_facts = await fact_repo.get_active_facts(TEST_USER_ID)
        ...
        # Mock LLM for Round 2 (Duplicate weight, New height)
        mock_llm_response_r2 = MagicMock()
        mock_llm_response_r2.text = f"""
        ```json
        {{
          "new_facts": [
            {{
        ...
        final_facts = await fact_repo.get_active_facts(TEST_USER_ID)
        ...
        # 4. CLEANUP
        report.log("\n--- STEP 4: Cleanup ---")
        await cleanup_test_data(TEST_USER_ID, db_client, env_config.firestore_collection_prefix, report, CLEANUP_ENABLED)
...
async def cleanup_test_data(user_id: str, db_client, prefix: str, report: E2EReport, enabled: bool):
    ...
    # 3. Delete facts
    facts_col = db_client.collection(f"{prefix}facts")
    facts = facts_col.where("owner_id", "==", user_id).stream()
    fcount = 0
    async_tasks = []
